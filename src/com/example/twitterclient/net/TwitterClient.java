...
public class TwitterClient extends OAuthBaseClient {
	...
	public TwitterClient(Context c) {
		super(c, Constants.REST_API_CLASS, Constants.REST_URL,
	...
	public void getTimeline(String query, long userId, String timelineType,
			long maxId, long sinceId, AsyncHttpResponseHandler handler) {
		String apiUrl;
		if (query != null && query.length() > 0) {
			apiUrl = getApiUrl(Constants.REST_SEARCH_TWEET_URL);
		} else {
			apiUrl = getApiUrl(String.format("statuses/%s_timeline.json",
					timelineType));
		}
	...
	public void getRelatedUsers(long userId, String relatedUserType,
			String nextCursor, AsyncHttpResponseHandler handler) {
		String apiUrl = getApiUrl(String
				.format("%s/list.json", relatedUserType));
		RequestParams params = new RequestParams();
		if (userId > 0) {
			params.put("user_id", Long.toString(userId));
		}
	...
	public void favoriteTweet(long tweetId, AsyncHttpResponseHandler handler) {
		String apiUrl = getApiUrl(Constants.REST_CREATE_FAVORITES_URL);
		RequestParams params = new RequestParams();
		if (tweetId > 0) {
	...
	public void unFavoriteTweet(long tweetId, AsyncHttpResponseHandler handler) {
		String apiUrl = getApiUrl(Constants.REST_DESTROY_FAVORITES_URL);
		RequestParams params = new RequestParams();
		if (tweetId > 0) {
			params.put("id", Long.toString(tweetId));
		}
