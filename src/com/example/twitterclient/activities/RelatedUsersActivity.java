...
public class RelatedUsersActivity extends AppActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		relatedUserType = (RelatedUserType) getIntent().getSerializableExtra(
				"relatedUserType");
		relatedUsersAdapter = new RelatedUsersAdapter(this);
		lvRelatedUsers.setAdapter(relatedUsersAdapter);
		getActionBar().setDisplayHomeAsUpEnabled(true);
		setTitle(relatedUserType == RelatedUserType.FRIENDS ? "Following"
				: "Followers");
		loadNextPage();
	}
	...
	private void performRequest(final boolean clearOnSuccess) {
		...
		getRelatedUsers(this, relatedUsersAdapter, userId, relatedUserType,
				clearOnSuccess, new Runnable() {
					@Override
					public void run() {
	...
	private static void getRelatedUsers(final Context context,
			final RelatedUsersAdapter adapter, final long userId,
			final RelatedUserType relatedUserType,
			final boolean clearOnSuccess, final Runnable completion) {
		final String relatedUser = relatedUserType == RelatedUserType.FRIENDS ? "friends"
				: "followers";
		TwitterClientApp.getClient().getRelatedUsers(userId, relatedUser,
				adapter.next_cursor, new JsonHttpResponseHandler() {
					@Override
					public void onSuccess(final JSONObject response) {
						if (!response.has("users"))
							return;
						try {
							final String nextCursor = response
									.has("next_cursor") ? response
									.getString("next_cursor") : null;
							final JSONArray jsonUsers = response
									.getJSONArray("users");
							Async.dispatch(new Runnable() {
								@Override
								public void run() {
									final ArrayList<User> users = User
											.fromJson(jsonUsers);
									...
									Async.dispatchMain(new Runnable() {
										...
										public void run() {
											...
											for (int i = 0, count = adapter
													.getCount(); i < count; ++i) {
												existingUserIds.add(adapter
														.getItem(i).userId);
											}
